...
const POLICIES: Policy[] = [
  {
    name: 'Ambetter_from_Sunshine_Health_Complete_Gold',
  ...
  {
    name: 'Florida Health Care Plans : Gym Access IND Essential Plus Silver HMO 53',
    states: ['Florida'],
  },
  {
    name: 'Florida Health Care Plans : Gym Access IND Platinum POS 4000',
    states: ['Florida'],
  },
];
...
const getPoliciesByState = (state: string): Policy[] => {
  return POLICIES.filter((p) =>
    p.states.some((s) => s.toLowerCase() === state.toLowerCase())
  );
};
...
export const useChatEngine = () => {
  ...
  const handleStateSelection = useCallback(async (state: string) => {
    const filteredPolicies = getPoliciesByState(state);

    setFlowContext((p) => ({
      ...p,
      state,
      availablePolicies: filteredPolicies,
    }));
    ...
    if (filteredPolicies.length === 0) {
      addBotMessage(
        `Sorry, we currently don’t have policies available in <strong>${state}</strong>. Please select another state.`
      );
      return;
    }

    addBotMessage(
      `Great! You've selected <strong>${state}</strong>.<br/><br/>
      Here are the available policies in your state:<br/>
      <ul>
        ${filteredPolicies.map((p) => `<li>${p.name}</li>`).join('')}
      </ul>
      <br/>Use the quick actions below to continue.`
    );
  ...
  const handlePolicySelect = useCallback(async (policy: string) => {
    const state = flowContext.state;
    const availablePolicies = flowContext.availablePolicies || [];

    const isValid = availablePolicies.some((p) => p.name === policy);

    if (!isValid) {
      addBotMessage(
        `⚠️ The selected policy is not available in <strong>${state}</strong>. Please choose a valid policy.`
      );
      return;
    }
    ...
    addBotMessage(
      `Thank you for choosing <strong>${policy}</strong>! Could you please share which benefits your customer is looking for?`
    );

    setInputEnabled(true);
    setFlowState('pitch-benefits');
  }, [flowContext, addBotMessage, simulateTyping]);

  const handleCompareSelect = useCallback(async (p1: string, p2: string) => {
    const availablePolicies = flowContext.availablePolicies || [];

    const isValid1 = availablePolicies.some((p) => p.name === p1);
    const isValid2 = availablePolicies.some((p) => p.name === p2);

    if (!isValid1 || !isValid2) {
      addBotMessage(
        `⚠️ One or both selected policies are not available in <strong>${flowContext.state}</strong>. Please select valid policies.`
      );
      return;
    }
    ...
    addBotMessage(
      `Excellent choices! I'll compare <strong>${p1}</strong> and <strong>${p2}</strong>.<br/><br/>
      First, which benefits is your customer most interested in?`
    );
