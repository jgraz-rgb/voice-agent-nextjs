...
import {sendOtp,verifyOtp} from './OTP_mcp_tools'

// ============================================================================
// TYPE DEFINITIONS
// ============================================================================

const aadhaarMobileMap: Record<string, string> = {
  "123456789012": "9876543210",
  "987654321098": "9123456789",
  // Add more Aadhaar → Mobile mappings as needed
};
...
const sendAadhaarOTPTool = tool({
  ...
  execute: async ({ aadhaar_number }: { aadhaar_number: string }) => {
    ...
    // 🔍 Lookup mobile number from dictionary
    const mobile_number = aadhaarMobileMap[normalized];

    if (!mobile_number) {
      return {
        success: false,
        message: "No mobile number registered for this Aadhaar number.",
      };
    }
    ...
    try {
      // 🔹 Use your MCP-based sendOtp helper to send the OTP
      const result = await sendOtp({ mobileNumber: mobile_number });
      console.log("✅ Aadhaar OTP Result:", result);

      return {
        success: result.msg?.toLowerCase().includes("success") ?? false,
        otp_reference_id,
        aadhaar_number: normalized,
        mobile_number,
        message: result.msg || "OTP sent successfully to registered number.",
      };
    } catch (err: any) {
...
const verifyAadhaarOTPTool = tool({
  ...
  parameters: z.object({
    ...
    aadhaar_number: z.string().describe("Aadhaar number used for sending OTP"),
  }),
  execute: async ({
    otp_code,
    aadhaar_number,
    ...
    try {
      ...
      const normalizedAadhaar = aadhaar_number.replace(/\s/g, "");

      // 🔍 Lookup mobile number from dictionary
      const mobile_number = aadhaarMobileMap[normalizedAadhaar];
      if (!mobile_number) {
        return {
          success: false,
          message: "No mobile number found for the given Aadhaar number.",
        };
      }

      // 🔹 Call MCP verify_otp tool
      ...
      if (result.otp_verified) {
        ...
        const mockAddress = stateManager.getState().city
          ? `101, Hill View, ${stateManager.getState().city} - ${stateManager.getState().pincode}`
          : "101, Hill View, Mumbai - 400101";

        stateManager.updateState({
          aadhaar_verified: true,
          aadhaar_address: mockAddress,
          aadhaar_dob: stateManager.getState().date_of_birth || "09/06/1992",
        });

        return {
          success: true,
          message: result.msg || "Aadhaar OTP verified successfully.",
          address: mockAddress,
          dob: stateManager.getState().date_of_birth || "09/06/1992",
          name: stateManager.getState().full_name || "Unknown",
        };
      }
      ...
      return {
        success: false,
        message: result.msg || "OTP verification failed. Please try again.",
      };
    } catch (err: any) {
      console.error("❌ Error verifying Aadhaar OTP:", err);
      return {
        success: false,
        message: err.message || "Error verifying Aadhaar OTP.",
      };
    }
  },
});
...
const sendGeneralOTPTool = tool({
  ...
  execute: async ({ phone_number }: { phone_number: string }) => {
    ...
    try {
      const result = await sendOtp({ mobileNumber: phone_number });
      console.log("OTP Result:", result);

      return {
        success: result.msg.toLowerCase().includes("success"),
        otp_reference_id,
        message: result.msg,
...
const verifyGeneralOTPTool = tool({
  ...
  parameters: z.object({
    ...
    otp_code: z.string().describe("6-digit OTP code provided by user"),
    phone_number: z.string().describe("Mobile number used for sending OTP"),
  }),
  execute: async ({
    otp_code,
    phone_number,
    ...
    try {
      const result = await verifyOtp({ mobileNumber: phone_number, user_input: otp_code });
