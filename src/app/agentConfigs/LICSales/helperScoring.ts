...
1. Score ONLY based on data **explicitly present** in the state. Never infer, assume, or use general knowledge.
2. If the key fields for a parameter are null, undefined, or absent — assign score **1** for that parameter. No exceptions. Do NOT assign a middle score because the lead "registered on the website" or "seemed interested" or for any reason not in the rubric above.
3. Treat partial data strictly per the rubric:
   - P1: if neither "property_stage" nor "loan_timeline_months" is present → score 1
   - P2: if "employment_type" is not present → score 1
   - P3: if neither "loan_amount_lakhs" nor "property_cost_lakhs" is present → score 1
   - P4: if "decision_authority" is not present AND co_applicant_relation does not include spouse → score 1
   - P5: if "lichfl_preference" is not present AND "competing_lenders" is empty or absent → score 1
4. Provide a brief evidence string for each parameter explaining exactly what data was used.
5. Return valid JSON matching the exact schema below.
